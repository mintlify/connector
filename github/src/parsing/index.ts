...
export const getFileSkeleton = (file: string, languageId: string): FileSkeleton | null => {
  try {
    ...
    const fileSkeleton = desiredPL.getFileSkeleton?.(fileTree.root) ?? null;
    return fileSkeleton;
  } catch {
    return null;
  }
