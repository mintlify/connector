...
export const fileToFileSkeleton = (file: GitbookFile, repo?: string, branch?: string, authConnector?: AuthConnectorType): FileSkeleton | null => {
    const languageId = getLanguageIdByFilename(file.filename);
    if (!SUPPORTED_LANGUAGES.includes(languageId)) return null;
...
export const fileToMdFile = (file: GitbookFile, repo?: string, branch?: string, authConnector?: AuthConnectorType): GitbookFile|null => {
...
export const filesToMdFiles = (files: GitbookFile[], repo?: string, branch?: string, authConnector?: AuthConnectorType, summary?: GitbookFile): GitbookFile[] => {
    const mdFiles: (GitbookFile|null)[] = [];
    files.forEach(async (file) => {
        const mdFile = fileToMdFile(file, repo, branch, authConnector);
        mdFiles.push(mdFile);
    });
    const filteredMdFiles = mdFiles.filter((file) => file != null) as GitbookFile[];
    if (summary != null) {
        filteredMdFiles.push(summaryUpdateOnInstallation(summary, filteredMdFiles));
    }
    return filteredMdFiles;
};
...
export const updateCodeFile = (filePair: FilePair): GitbookFile => {
    ...
    mdSkeletons.forEach((skeleton) => {
        ...
        if (matchingSkeleton != null && matchingSkeleton?.rawDoc != null && skeleton?.doc != null && skeleton.doc !== matchingSkeleton.doc) {
...
export const updateMdFile = (filePair: FilePair): GitbookFile => {
    const { md } = filePair;
    const mdSkeletons: Skeleton[] = filePair.md.fileSkeleton?.skeletons ?? [];
    if (md === null && filePair.code.fileSkeleton != null) {
    ...
    codeSkeletons.forEach((skeleton) => {
        ...
        if (skeletonWithSignatureMatch != null && skeletonWithSignatureMatch?.doc != null && skeleton?.doc != null && skeleton.doc !== skeletonWithSignatureMatch.doc) {
