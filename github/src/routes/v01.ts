...
export const getAuthConnector = async (sourceId: string): Promise<AuthConnectorType|undefined> => {
    const hashedSourceId = sha512Hash(sourceId);
    const authConnector = await AuthConnector.findOne({hashedSourceId});
    return authConnector ?? undefined;
}
...
v01Router.get('/integrations', async (req, res) => {
    ...
    const authConnectorResponse = await getAuthConnector(owner);
    const gitbook = authConnectorResponse?.gitbook;
    const alerts = authConnectorResponse?.alerts;
    return res.status(200).send({
        gitbook,
        alerts
    })
