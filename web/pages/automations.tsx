...
export default function Automations({ userSession }: { userSession: UserSession }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_ENDPOINT}/routes/automations`, {
        params: {
          userId: user.userId,
  ...
  const handleToggleSwitch = async (automationId: string, isActive: boolean) => {
    axios
      .put(
        `${API_ENDPOINT}/routes/automations/active`,
      ...
      .then(() => {
        const newAutomations = automations.map((automation) =>
          automation._id === automationId ? { ...automation, isActive } : automation
        )
        ...
        <div className="flex-grow w-full max-w-7xl mx-auto xl:px-8 lg:flex">
          ...
          <div className="flex-1 min-w-0 xl:flex">
            ...
            <div className="bg-white lg:min-w-0 lg:flex-1">
              ...
              {!hasAutomations && !isLoading && (
                <div className="pb-8">
                  <div className="flex items-center justify-center">
                    <img className="w-24 h-24" src="/assets/empty/automations.svg" alt="Empty Automations" />
                  </div>
                  <p className="text-center mt-6 text-gray-600 font-medium">No automations created</p>
                      ...
                      <div className="px-4 py-5 sm:px-6">
                        <div className="flex items-center justify-between">
                          <div className="flex space-x-2 items-center">
                            <AutomationTypeIcon type={automation.type} outerSize={8} innerSize={5} />
                        ...
                        <div className="mt-2 sm:flex sm:justify-between">
                          <div className="sm:flex">
                            <p className="flex items-center text-sm text-gray-500">
                              <TypeIcon type={automation.type} className="flex-shrink-0 mr-1 h-4 w-4 text-gray-400" />
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {integrations.map((integration) => (
                  <Link key={integration.id} href={integration.href}>
                    <div className="relative rounded-md border border-gray-200 bg-white px-3 py-2 shadow-sm flex items-center space-x-3 hover:bg-gray-50">
                          ...
                          <div className="flex space-x-0.5 items-center">
                            <p className="text-xs text-gray-500 truncate">
                              {integrationsStatus == null ? (
                                <Skeleton width={56} />
                              ) : integrationsStatus[integration.id] ? (
                                'Installed'
                              ) : (
                                'Not installed'
                              )}
                            </p>
                            {integrationsStatus != null && integrationsStatus[integration.id] ? (
