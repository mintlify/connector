...
export default function Home({ userSession }: { userSession: UserSession }) {
	...
	useEffect(() => {
		...
		axios.get(eventsQuery).then((eventsResponse) => {
	...
	} else if (userSession.user.pending)
		return (
			<Setup
									...
									<div key={doc._id}>
										...
										<li
											className={classNames(
												"relative pl-4 pr-6 py-5 hover:bg-gray-50 sm:pl-6 lg:pl-8 xl:pl-6 cursor-pointer",
												doc._id === selectedDoc?._id ? "bg-gray-50" : ""
											)}
												...
												<div className="min-w-0 space-y-2">
													...
													<a className="relative group flex items-center space-x-2.5">
														<span className="flex items-center space-x-2.5 text-sm text-gray-500 truncate">
															...
															<div>Last updated {timeAgo.format(Date.parse(doc.lastUpdatedAt))}</div>
														</span>
													</a>
												</div>
															...
															{doc.automations.map((automation) => (
																<Tooltip key="auto" message={automation.name}>
																	<Link href="/automations">
																		<a className="hover:opacity-50">{getAutomationTypeIcon(automation.type, 6, 4)}</a>
																	</Link>
																</Tooltip>
															))}
														</div>
														{[].length > 5 ? <span className="flex-shrink-0 text-xs leading-5 font-medium">+1</span> : null}
													</div>
												</div>
											</div>
										</li>
									...
									{events?.map((event) => (
										<li key={event._id} className="py-4">
											<div className="flex space-x-3">
												<img className="h-5 w-5 rounded-sm" src={event.doc.favicon} alt={event.doc.title} />
													...
													{event.change && (
														<div>
															<div className="flex items-center space-x-1 text-green-700">
																...
																<p className="text-sm">
																	{countTotalChanges(event.change).added}{" "}
																	{countTotalChanges(event.change).added > 1 ? "changes" : "change"}
																</p>
															</div>
															<div className="flex items-center space-x-1 text-red-700">
																<MinusIcon className="h-4 w-4" />
																<p className="text-sm">
																	{countTotalChanges(event.change).removed}{" "}
																	{countTotalChanges(event.change).removed > 1 ? "changes" : "change"}
																</p>
															</div>
														</div>
													)}
