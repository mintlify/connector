...
export namespace GitReference {
    ...
    export function toString(
        ...
        if (!Array.isArray(refs) || refs.length === 1) {
            ...
            switch (ref.refType) {
                case 'branch':
                    result = `${options.label ? `${ref.remote ? 'remote ' : ''}branch ` : ''}${options.icon ? `$(git-branch)${GlyphChars.Space}${refName}` : refName
                        }`;
                    break;
                case 'tag':
                ...
                default: {
                    if (GitReference.isStash(ref)) {
                        ...
                        if (options.expand && ref.message) {
                            message = `${ref.number != null ? `${ref.number}: ` : ''}${ref.message.length > 20
                                ? `${ref.message.substring(0, 20).trimRight()}${GlyphChars.Ellipsis}`
                                : ref.message
                                }`;
                        }

                        result = `${options.label ? 'stash ' : ''}${options.icon
                        ...
                        result = `${options.label ? `${prefix}commit ` : ''}${options.icon
